...
-- reading the audit log is therefore not itself a recursive PII read. `id` is minted by
-- `gen_random_uuid()` (core since PG13 — no pgcrypto) called INLINE in the audit INSERT (T07) — the
-- same way every other table's store INSERT mints its id (see server/store/src/lib.rs), so the Rust
-- core stays randomness-free (ADR-0021) and there is NO column DEFAULT (consistent with the whole
-- schema — no table carries one). Group-scoped ENABLE+FORCE RLS like every PII table, so AC16's
-- cross-tenant proof covers it too.
--
...
CREATE TABLE audit_log (
    id         uuid        PRIMARY KEY,        -- minted by gen_random_uuid() inline in the audit INSERT (T07)
