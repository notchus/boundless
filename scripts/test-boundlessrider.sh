#!/usr/bin/env bash
#
# test-boundlessrider.sh — run the Rider onboarding UI tests on the iOS simulator (spec 001 T11):
# snapshot ×4 a11y variants per screen + the VoiceOver-order / no-signup / no-toggle / AC5/AC8/AC14/
# AC15 logic tests. Builds the BoundlessKit XCFramework first (BoundlessRider depends on it).
#
# To (re)record snapshot baselines after an intentional UI change:
#   TEST_RUNNER_SNAPSHOT_RECORD=1 bash scripts/test-boundlessrider.sh   # records, then re-run to verify
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PKG="$REPO_ROOT/apple/BoundlessRider"

if [[ -z "${DEVELOPER_DIR:-}" && -d /Applications/Xcode.app ]]; then
  export DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer
fi

# Pick a simulator: honor $BOUNDLESS_SIM, else the first available iPhone (robust across Xcode
# versions / CI runner images, where the exact model name varies).
SIM="${BOUNDLESS_SIM:-}"
if [[ -z "$SIM" ]]; then
  SIM="$(xcrun simctl list devices available \
    | sed -nE 's/^[[:space:]]+(iPhone[^()]*) \([0-9A-Fa-f-]+\) \(.*/\1/p' \
    | head -1 | sed 's/[[:space:]]*$//')"
  [[ -n "$SIM" ]] || {
    echo "error: no available iPhone simulator found. Install one in Xcode, or set BOUNDLESS_SIM." >&2
    exit 1
  }
fi
echo "▸ simulator: $SIM"

# BoundlessRider depends on the BoundlessKit XCFramework (a build artifact) — build it first.
bash "$REPO_ROOT/scripts/build-boundlesskit.sh"

echo "▸ xcodebuild test (RiderShared) on '$SIM'"
cd "$PKG"
xcodebuild test \
  -scheme BoundlessRider \
  -destination "platform=iOS Simulator,name=$SIM" \
  -derivedDataPath "$PKG/.build-xcode"
