...
| `ADMIN_MEMBER_NOT_FOUND` | A detail read / edit / regenerate-code named a member id that does not exist in the Group (a plain 404). No PII is read and no audit row is written. The core models this as a no-body outcome; the Worker returns it as the stable 404 `ErrorBody` code for the admin UI. | detail / edit / regenerate · AC7/AC11 | `admin.member.not_found` (UI copy authored at T10) | yes (refresh the list) |
| `ADMIN_UNAUTHORIZED` | The `/api/admin/*` request carried no / a wrong `ADMIN_API_SECRET` shared secret — the ADR-0026 BFF gate failed closed (401). Not a client-facing error (only the trusted SvelteKit BFF calls these routes); surfaces a BFF misconfiguration. The body carries only the code (no detail). | admin BFF gate · ADR-0026 | — (no client surface; a BFF/deploy fix) | no (fix the secret) |
| `ADMIN_BAD_REQUEST` | An `/api/admin/*` request was malformed — an unparseable JSON body or a missing/invalid `X-Admin-Id` (400). The submitted value is never echoed (P2/R10). | admin request validation · ADR-0026 | — (no client surface; a BFF fix) | yes (fix the request) |
