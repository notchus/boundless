...
|---|---|---|
| `POST /api/admin/webauthn/invite/resolve` | Resolve a presented token → pending-admin invite metadata (the **core** computes the token's keyed hash; matched by unique-index equality). Read-only; edge-TS then applies the TTL/consumed verdict. | `adminSharedSecret` **only** (pre-session) |
...
- **The token match stays in the core (P4, AC4b).** `invite/resolve` computes the token's keyed at-rest hash via `core::crypto::admin_invitation_token_hash` (domain-tagged HMAC) and matches it by equality on the unique `token_hash` index — never re-implemented in edge-TS. As built (spec 009 T02), that indexed equality *is* the timing-safe compare: the value compared is a secret-keyed 256-bit HMAC (not the token itself), so a row-by-row constant-time byte compare (`admin_invitation_token_matches`) is unnecessary on this lookup path — the same pattern as `find_member_by_phone` / `classify_refresh`. (The constant-time `admin_invitation_token_matches` remains in `core/crypto` for any scan-style compare; the resolve path uses the indexed-hash lookup instead.) Keeping the hash in Rust — not edge-TS — is the whole point of B1 over B2.
...
- `core/crypto/src/hashing.rs` — `admin_invitation_token_hash` (the domain-tagged keyed hash `invite/resolve` computes in the core; matched by unique-index equality, timing-safe on a secret-keyed HMAC). `admin_invitation_token_matches` (constant-time byte compare) remains available but is not needed on the indexed-lookup path.
