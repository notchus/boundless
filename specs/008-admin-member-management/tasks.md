...
| AC16 cross-tenant deployed-edge proof (F5) | T07 (store precursor), T11 (Worker-HTTP precursor + operator handoff), T11 [shell] live | **T07 ✓ store precursor** (`rls_isolates_member_reads_by_tenant` + `prop_rls_isolates_random_two_group_configs`, superuser+`SET ROLE`); **T11 ✓ Worker-HTTP precursor** (`worker_cross_tenant_admin_cannot_read_other_group`: a Group-A-scoped Worker, as the NOBYPASSRLS `boundless_app` role over real PG18, cannot read/list/edit/regenerate a seeded Group-B member); **live deployed-edge run ✓ DONE 2026-06-12** — the operator provisioned the deployed Neon (idempotent `provision-neon.sh` applied 0009–0011 + re-granted), bootstrapped Group A (`bootstrap-group.sh`), seeded a throwaway Group-B probe row, and ran `smoke-deployed-edge.sh` with `ADMIN_API_SECRET` + `CROSS_TENANT_MEMBER_ID` against the live edge → the Group-A admin got a clean **404** (cannot read/list the Group-B member). RLS isolation proven end-to-end in production; **AC16 fully closed** |
