//! `boundless-ffi-kotlin` — UniFFI binding crate that produces the `BoundlessCore` AAR for the
//! Android platform (docs/architecture.md §1; spec 001 Android bring-up, mirrors the Swift leg).
//!
//! ## What this crate is (and is not)
//! It is one of the **two** places UniFFI touches Boundless (the other is `core/ffi-swift`). The
//! wasm-safe core crates (`boundless-domain`, `boundless-auth`) stay free of any `uniffi`
//! dependency so they keep compiling to `wasm32-unknown-unknown`; UniFFI cannot be added to them.
//! Therefore this crate **mirrors** the client-relevant core enums with `#[derive(uniffi::Enum)]`
//! and bridges them with **exhaustive `From` conversions** in both directions. The exhaustive
//! `match` is the parity guard: if a core variant is added or renamed, this crate fails to compile
//! until the conversion is updated — so the generated Kotlin can never silently drift from the core
//! (P4). This is a sanctioned codegen adapter, not a hand-rolled duplicate. See **ADR-0022**.
//!
...
//!
//! Consumed by the Android `core-bridge` module. Generated bindings + the `.so` jniLibs are
//! produced by `scripts/build-corebridge.sh` and are NOT committed (reproducible from this source).
...
/// tuple variants are mirrored as named-field variants for a clearer Kotlin surface.
#[derive(uniffi::Enum, Clone, Copy, Debug, PartialEq, Eq)]
